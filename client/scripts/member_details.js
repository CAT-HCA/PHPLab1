...
$(function() {
	...
	$("#editViewMemberBtn").on("click", function() {
		window.location.assign(
			`edit_member.php?id=${sectionId}&name=${sectionName}&code=${leagueCode}&memberid=${memberId}`
		);
	});
	...
	$("#deleteViewMemberBtn").on("click", function() {
		$.ajax({
			url: "http://localhost:8081/api/teams/" + sectionId + "/members/" + memberId,
			method: "DELETE",
			success: function(result) {
				alert("The member has been successfully deleted!");
				window.location.assign(
					"team_details.php?id=" + sectionId + "&name=" + sectionName + "&code=" + leagueCode
				);
			},
		});
	});
