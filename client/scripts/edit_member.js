...
$(function() {
	...
	$("#cxlEditMemberBtn").on("click", function() {
		window.location.assign("team_details.php?id=" + sectionId + "&name=" + sectionName + "&code=" + leagueCode);
	});
	...
	$.getJSON("http://localhost:8081/api/teams/" + sectionId + "/members/" + memberId, function(data) {
		...
		$("#cxlEditMemberBtn").on("click", function() {
			window.location.assign(
				"team_details.php?id=" + teamId + "&name=" + data.TeamName + "&code=" + data.League
			);
		});
	});
});
...
function finishEditMember(teamId, sectionName, leagueCode) {
	$.ajax({
		...
		success: function(result) {
			window.location.assign("team_details.php?id=" + teamId + "&name=" + sectionName + "&code=" + leagueCode);
		},
	});
}
