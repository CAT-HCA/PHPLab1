...
$(document).ready(function() {
	...
	$.getJSON("http://localhost:8081/api/leagues/" + leagueCode, function(data) {
		$("#instFamId").html(data.Name);
	});
	//filling in section info
	$.getJSON("http://localhost:8081/api/teams/" + sectionId, function(data) {
	...
	$("#deleteTeam").on("click", function() {
		$.ajax({
			url: "http://localhost:8081/api/teams/" + sectionId,
			method: "DELETE",
	...
	$("#sectInfoEdit").attr("href", "edit_section.php?id=" + sectionId + "&focus=editSectionId");
	$("#membershipInfoEdit").attr("href", "edit_section.php?id=" + sectionId + "&focus=editSectionMaxMems");
	$("#sectLeaderEdit").attr("href", "edit_section.php?id=" + sectionId + "&focus=editSectionManagerName");
	$("#sectPhotoEdit").attr("href", "edit_section.php?id=" + sectionId + "&focus=editSectionPhotoUpload");
});
...
function createMemberRow(memberName, memberId, sectionId, sectionName, leagueCode, i) {
	...
	$("#membershipList" + i).append(
		`${memberName} <a href="edit_member.php?id=${sectionId}&name=${sectionName}&code=${leagueCode}&memberid=${memberId}" class="float-right mx-2"><i class="far fa-edit float-right"></i></a>
<a href="member_details.php?id=${sectionId}&name=${sectionName}&code=${leagueCode}&memberid=${memberId}" class="float-right mx-2"><i class="fas fa-info-circle float-right"></i></a>`
	);
}
//function to assign side nav bar links
function assignSideNavLinks(sectionName, sectionId, leagueCode) {
	$("#editTeamDetails").attr("href", "edit_section.php?id=" + sectionId + "&focus=none");
	$("#addTeamMember").attr(
		"href",
		"add_member.php?id=" + sectionId + "&name=" + sectionName + "&code=" + leagueCode
	);
}
